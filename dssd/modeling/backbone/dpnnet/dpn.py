...
class BnActConv2d(nn.Module):
    def __init__(self, in_chs, out_chs, kernel_size, stride,
                 padding=0, groups=1, activation_fn=nn.ReLU(inplace=True), dilation=False):
        super(BnActConv2d, self).__init__()
        self.bn = nn.BatchNorm2d(in_chs, eps=0.001)
        self.act = activation_fn
        if dilation:
            self.conv = nn.Conv2d(in_chs, out_chs, kernel_size, stride, padding=2, groups=groups, dilation=(2, 2), bias=False)
        else:
...
class AvgPoolingBnActConv2d(nn.Module):
    def __init__(self, in_chs, out_chs, kernel_size, stride,
                 padding=0, groups=1, activation_fn=nn.ReLU(inplace=True)):
        super(AvgPoolingBnActConv2d, self).__init__()
        self.bn = nn.BatchNorm2d(in_chs, eps=0.001)
        self.act = activation_fn
        self.conv = nn.Conv2d(in_chs, out_chs, kernel_size, stride, padding, groups=groups, bias=False)
        self.avgpooling = nn.AvgPool2d(3,1)


    def forward(self, x):
        x = self.avgpooling(self.act(self.bn(x)))
        return self.conv(self.act(self.bn(x)))
...
class DualPathBlock(nn.Module):
    def __init__(
            self, in_chs, num_1x1_a, num_3x3_b, num_1x1_c, inc, groups, block_type='normal', b=False, dilation=False, final=False):
        # 64, 96, 96, 256, 16, 32, 'proj', False
        ...
        if self.has_proj:
            ...
            if self.key_stride == 2:
                if final:
                    self.c1x1_w_s2 = AvgPoolingBnActConv2d(
        ...
        if final:
            self.c3x3_b = BnActConv2d(
                ...
                stride=1, padding=0, groups=groups, dilation=dilation)
        else:
            self.c3x3_b = BnActConv2d(
                in_chs=num_1x1_a, out_chs=num_3x3_b, kernel_size=3,
                stride=self.key_stride, padding=1, groups=groups, dilation=dilation)
...
class DPN(nn.Module):
    def __init__(self, small=False, num_init_features=64, k_r=96, groups=32,
                 b=False, k_sec=(3, 4, 20, 3), inc_sec=(16, 32, 24, 128), predict_layer=(-5,-4,-3,-2,-1)):
        ...
        # conv2
        bw = 64 * bw_factor   # 64*4=256
        inc = inc_sec[0]     # 16
        r = (k_r * bw) // (64 * bw_factor)  # 96
        blocks['conv2_1'] = DualPathBlock(num_init_features, r, r, bw, inc, groups, 'proj', b)
        in_chs = bw + 3 * inc  # 256+16*3=304
        for i in range(2, k_sec[0] + 1):
            blocks['conv2_' + str(i)] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'normal', b)
            in_chs += inc

        # conv3
        bw = 128 * bw_factor  # 128*4=512
        inc = inc_sec[1]
        r = (k_r * bw) // (64 * bw_factor)  # 192
        blocks['conv3_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'down', b)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[1] + 1):
            blocks['conv3_' + str(i)] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'normal', b)
            in_chs += inc
        self.cat.add_module('cat1', CatBnAct(in_chs))

        # conv4
        bw = 256 * bw_factor  # 256*4=1024
        inc = inc_sec[2]
        r = (k_r * bw) // (64 * bw_factor)  # 384
        ...
        bw = 256 * bw_factor  # 256*4=1024
        inc = inc_sec[3]
        r = (k_r * bw) // (64 * bw_factor)  # 768
        blocks['extra_conv5_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'proj', b, dilation=False)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[3] + 1):
            blocks['extra_conv5_' + str(i)] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'normal', b, dilation=False)
            in_chs += inc

        self.cat.add_module('cat2', CatBnAct(in_chs))

        # conv6
        bw = 256 * bw_factor  # 256*4=1024
        inc = inc_sec[3]
        r = (k_r * bw) // (64 * bw_factor)  # 768
        blocks['conv6_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'down', b, dilation=False)
        in_chs = bw + 3 * inc

        self.cat.add_module('cat3', CatBnAct(in_chs))

        # conv7
        bw = 256 * bw_factor  # 256*4=1024
        inc = inc_sec[3]
        r = (k_r * bw) // (64 * bw_factor)  # 768
        blocks['conv7_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'down', b, dilation=False)
        in_chs = bw + 3 * inc

        self.cat.add_module('cat4', CatBnAct(in_chs))

        # conv8
        bw = 256 * bw_factor  # 256*4=1024
        inc = inc_sec[3]
        r = (k_r * bw) // (64 * bw_factor)  # 768
        blocks['conv8_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'down', b, dilation=False)
        in_chs = bw + 3 * inc

        self.cat.add_module('cat5', CatBnAct(in_chs))

        # conv9
        bw = 256 * bw_factor  # 256*4=1024
        inc = inc_sec[3]
        r = (k_r * bw) // (64 * bw_factor)  # 768
        blocks['conv9_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'down', b, final=True)
        in_chs = bw + 3 * inc

        self.cat.add_module('cat6', CatBnAct(in_chs))
    ...
    def forward(self, input):
        x = self.features[0: -27](input)
        soureces = []
        soureces += [x]
        for i in range(-27, 0):
            x = self.features[i](x)
            if i in self.predict_layer:
                soureces += [x]
        for i in range(0, 6):
            soureces[i] = self.cat[i](soureces[i])
        return soureces
